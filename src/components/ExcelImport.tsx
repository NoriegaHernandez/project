      ...
      for (let i = 0; i < records.length; i++) {
        ...
        try {
          ...
          const { data: existingStudent } = await supabase
            .from('estudiantes')
            .select('id')
            .eq('numero_control', record.numero_control)
            .maybeSingle();
          ...
          if (existingStudent) {
            ...
            const { data: newStudent, error: studentError } = await supabase
              .from('estudiantes')
              .insert({
                numero_control: record.numero_control,
                nombre: record.nombre,
                apellido_paterno: record.apellido_paterno,
                apellido_materno: record.apellido_materno,
                semestre_actual: parseInt(record.semestre_actual) || 1,
              })
              .select()
              .single();
          ...
          const { data: materia } = await supabase
            .from('materias')
            .select('id')
            .eq('nombre', record.materia)
            .maybeSingle();

          if (!materia) {
            errors.push(`Fila ${i + 2}: Materia "${record.materia}" no encontrada`);
            continue;
          }

          // Procesar calificaciones
          const unidad1 = parseFloat(record.calificacion_unidad1) || null;
          const unidad2 = parseFloat(record.calificacion_unidad2) || null;
          const unidad3 = parseFloat(record.calificacion_unidad3) || null;

          let calificacionFinal = null;
          if (unidad1 !== null && unidad2 !== null && unidad3 !== null) {
            calificacionFinal = (unidad1 + unidad2 + unidad3) / 3;
          }

          const estado = calificacionFinal && calificacionFinal >= 70 ? 'aprobado' : 'reprobado';

          // Insertar registro académico
          await supabase.from('registros_estudiante_materia').insert({
            estudiante_id: studentId,
            materia_id: materia.id,
            semestre: parseInt(record.semestre_cursado) || 1,
            calificacion_unidad1: unidad1,
            calificacion_unidad2: unidad2,
            calificacion_unidad3: unidad3,
            calificacion_final: calificacionFinal,
            porcentaje_asistencia: parseFloat(record.porcentaje_asistencia) || null,
            estado,
          });
    ...
    <div className="bg-white rounded-lg shadow-lg p-6">
      ...
      <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
        ...
        <div className="text-sm text-blue-800 space-y-1">
          <p><strong>Columnas requeridas:</strong></p>
          <ul className="list-disc list-inside ml-4 space-y-1">
            <li><code>numero_control</code> - Número de control del estudiante</li>
            <li><code>nombre</code> - Nombre(s) del estudiante</li>
            <li><code>apellido_paterno</code> - Apellido paterno</li>
            <li><code>apellido_materno</code> - Apellido materno</li>
            <li><code>semestre_actual</code> - Semestre actual del estudiante</li>
            <li><code>materia</code> - Nombre de la materia</li>
            <li><code>semestre_cursado</code> - Semestre en que cursó la materia</li>
            <li><code>calificacion_unidad1</code> - Calificación unidad 1</li>
            <li><code>calificacion_unidad2</code> - Calificación unidad 2</li>
            <li><code>calificacion_unidad3</code> - Calificación unidad 3</li>
            <li><code>porcentaje_asistencia</code> - Porcentaje de asistencia</li>
          </ul>
          <p className="mt-3"><strong>Ejemplo de encabezado:</strong></p>
          <code className="block bg-white p-2 rounded text-xs mt-1">
            numero_control,nombre,apellido_paterno,apellido_materno,semestre_actual,materia,semestre_cursado,calificacion_unidad1,calificacion_unidad2,calificacion_unidad3,porcentaje_asistencia
          </code>
        </div>
      </div>
