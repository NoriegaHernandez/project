...
export default function Reports() {
  ...
  const loadReportData = async () => {
    ...
    try {
      ...
      const failed = records?.filter(r => r.estado === 'reprobado').length || 0;
      const dropout = records?.filter(r => r.estado === 'baja').length || 0;
      const approved = records?.filter(r => r.estado === 'aprobado').length || 0;
      ...
      const factorCounts = new Map<string, number>();
      riskFactors?.forEach((rf: any) => {
        const name = rf.factores_riesgo?.nombre || 'Desconocido';
        factorCounts.set(name, (factorCounts.get(name) || 0) + 1);
      });
