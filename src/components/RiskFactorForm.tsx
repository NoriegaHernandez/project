...
// import { supabase, CategoriaFactorRiesgo, FactorRiesgo, FactorRiesgoEstudiante } from '../lib/supabase';
...
//   const [categories, setCategories] = useState<CategoriaFactorRiesgo[]>([]);
...
//                   >
//                     {category.nombre} {selectedCategories.includes(category.id) ? '(Ya agregado)' : ''}
//                   </option>
//                 ))}
//               </select>
//             </div>
...
export default function RiskFactorForm({ recordId, studentName, onSuccess, onCancel }: RiskFactorFormProps) {
  ...
  const loadData = async () => {
    ...
    const { data: existing } = await supabase
      ...
      `)
      .eq('registro_estudiante_materia_id', recordId);

    if (cats) setCategories(cats);
    if (existing) {
      const categoryIds = existing.map((e: any) => e.factores_riesgo.categoria_id);
      setSelectedCategories([...new Set(categoryIds)]);
    }
  };
  ...
  const handleAddFactor = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error: insertError } = await supabase.from('factores_riesgo_estudiante').insert({
        registro_estudiante_materia_id: recordId,
      ...
      setSelectedCategories([...selectedCategories, formData.categoria_id]);
      setFormData({ categoria_id: '', severidad: 'media', observaciones: '' });
    } catch (error) {
      console.error('Error al agregar factor de riesgo:', error);
      alert('Error al agregar factor de riesgo');
    } finally {
      setLoading(false);
    }
        ...
        <div className="p-6 space-y-6">
          ...
          <form onSubmit={handleAddFactor} className="space-y-4">
            ...
            <div>
              ...
              <div className="flex gap-4">
                {(['baja', 'media', 'alta'] as const).map((severidad) => (
                  <label key={severidad} className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="radio"
                      name="severidad"
                      value={severidad}
                    ...
                    />
                    <span className="text-sm text-gray-700 capitalize">
                      {severidad === 'baja' ? 'Baja' : severidad === 'media' ? 'Media' : 'Alta'}
                    </span>
                  </label>
                ))}
              </div>
