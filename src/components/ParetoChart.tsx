...
export default function ParetoChart({ filters }: ParetoChartProps) {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (filters.semester) {
        query = query.eq('registros_estudiante_materia.semestre', filters.semester);
      }

      if (filters.majorId) {
        query = query.eq('registros_estudiante_materia.estudiantes.carrera_id', filters.majorId);
      }
      ...
      riskData?.forEach((item: any) => {
        const factorName = item.factores_riesgo?.nombre || 'Desconocido';
        factorCounts.set(factorName, (factorCounts.get(factorName) || 0) + 1);
      });
